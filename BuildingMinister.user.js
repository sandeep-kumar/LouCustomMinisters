        ...
        qx.Class.define("CustomBuildingMinister", {
            ...
            members: {
                ...
                fillTheBuildingQueueOFCurrentCity: function() {
                    ...
                    LouB.removeAllResourceDestructionInBQ();
                ...
                removeAllResourceDestructionInBQ: function() {
                    var buildingQueue = LouB.cityInfo.buildQueue;
                    if(!buildingQueue)
                        return;
                    for(var i=0; i<buildingQueue.length; i++){
                        if((buildingQueue[i].type>=27 && buildingQueue[i].type<=30) && buildingQueue[i].level === 1) {
                            var commandManager = webfrontend.net.CommandManager.getInstance();
                            var buildingId = buildingQueue[i].building;
                            var queueId = buildingQueue[i].id;
                            var cityId = LouB.cityInfo.getId();
                            commandManager.sendCommand("CancelBuild", {
                                cityid: cityId,
                                bid: buildingId,
                                queueid: queueId
                            }, this, function () {});
